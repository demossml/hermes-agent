...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _handle_snapshot_command(self, cmd_original: str):
        """Handle /snapshot — manage state snapshots."""
    ...
    def _handle_reasoning_command(self, cmd_original: str):
        """Handle /reasoning — set reasoning effort or toggle display."""
        parts = cmd_original.strip().split()
        if len(parts) < 2:
            # Show current state
    ...
    def _handle_mode(self, cmd: str):
        ...
        elif sub == 'project' and len(parts) > 2:
            self._handle_project(f'/project switch {parts[2]}')
        else:
            _cprint('  Usage: /mode neutral  |  /mode project <name>')
